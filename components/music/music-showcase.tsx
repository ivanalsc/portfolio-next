    ...
    <div className="relative h-48">
      <div className="absolute inset-0 flex justify-center">
        {albums.map((album, index) => {
          ...
          // Cambiamos el cálculo de la posición para evitar el scroll horizontal
          const leftPosition = index * 16 - (albums.length * 5); // Distribuimos los elementos más juntos
          ...
          return (
            <motion.div
              ...
              style={{
                left: `calc(30% + ${leftPosition}px)`, // Centramos y ajustamos posición relativa
                top: `${Math.random() * 20}px`,
                transformOrigin: "center bottom",
                width: "32%", // Limitamos el ancho máximo en móviles
                maxWidth: "120px", // Ponemos un límite máximo
              }}
              initial={{
                rotate: rotateValue,
              ...
              animate={{
                ...
                scale: selectedAlbum?.id === album.id ? 1.1 : 1, // Reducimos un poco la escala
                y: selectedAlbum?.id === album.id ? -20 : 0,
                zIndex: selectedAlbum?.id === album.id ? 50 : albums.length - index,
                transition: { duration: 0.3 }
              }}
              whileHover={{
                scale: 1.1, // Reducimos un poco la escala en hover
                y: -15,
                zIndex: 40,
